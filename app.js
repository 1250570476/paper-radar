...
const scanTips=["Searching title, abstract, and topic metadata—not titles alone.","Testing scientific synonyms helps find papers that use different terminology.","The final relevance ranking happens only after discovery is complete.","Every paper in the selected journals and date window is included in the census.","Duplicate records from different query paths are merged automatically."];
let scanTipTimer=null,scanTipIndex=0;
function scanLog(message){const log=$("#scan-log");if(!log||!message)return;const item=document.createElement("li");item.textContent=message;log.prepend(item);while(log.children.length>4)log.lastElementChild.remove();}
function openScan(totalQueries,journalCount){$("#search-progress").classList.remove("hidden","minimized");$(".scan-body").classList.remove("hidden");$("#expand-scan").classList.add("hidden");$("#scan-log").innerHTML="";$("#scan-progress-bar").style.width="3%";$("#scan-percent").textContent="3%";$("#scan-mini-percent").textContent="3%";$("#scan-journals").textContent=journalCount||"All";$("#scan-queries").textContent=totalQueries||0;$("#scan-candidates").textContent="0";$("#scan-title").textContent="Mapping your research interests";$("#scan-stage").textContent="Building a multi-pass search plan…";$("#scan-current-label").textContent="CURRENT ACTIVITY";$("#scan-current-value").textContent="Identifying concepts, phrases, and scientific synonyms";scanLog("Research profile converted into a structured search plan");clearInterval(scanTipTimer);scanTipIndex=0;$("#scan-tip").textContent=scanTips[0];scanTipTimer=setInterval(function(){scanTipIndex=(scanTipIndex+1)%scanTips.length;$("#scan-tip").textContent=scanTips[scanTipIndex];},4500);}
function updateScan(data){data=data||{};if(data.progress!=null){const p=Math.max(3,Math.min(100,Math.round(data.progress)));$("#scan-progress-bar").style.width=p+"%";$("#scan-percent").textContent=p+"%";$("#scan-mini-percent").textContent=p+"%";}if(data.title)$("#scan-title").textContent=data.title;if(data.stage)$("#scan-stage").textContent=data.stage;if(data.current)$("#scan-current-value").textContent=data.current;if(data.label)$("#scan-current-label").textContent=data.label;if(data.candidates!=null)$("#scan-candidates").textContent=Number(data.candidates).toLocaleString();if(data.queries!=null)$("#scan-queries").textContent=data.queries;if(data.journals!=null)$("#scan-journals").textContent=data.journals||"All";if(data.log)scanLog(data.log);$("#scan-mini-text").textContent=data.current||data.stage||"Scanning papers…";}
function closeScan(success){clearInterval(scanTipTimer);updateScan({progress:100,title:success?"Scan complete":"Scan paused",stage:success?"Ranking complete—your feed is ready.":"The literature service could not complete this scan.",current:success?"Relevant papers have been added to your feed":"Try the scan again shortly",log:success?"Final relevance ranking completed":"Scan ended before completion"});setTimeout(function(){$("#search-progress").classList.add("hidden");},success?1400:3500);}
...
async function fetchTargeted(ids,date,plan,onProgress){
  ...
  for(let offset=0;offset<plan.length;offset+=batchSize){
    ...
    onProgress?.(Math.min(offset+batch.length,plan.length),plan.length,batch[batch.length-1]);
  }
  return works;
}
async function fetchWorks(){updateScan({progress:8,title:"Searching for research matches",stage:"Pass 1 of 3 · Targeted discovery"});const days=Number($("#days-select").value),date=new Date(Date.now()-days*DAY).toISOString().slice(0,10),favorites=state.profile.favoriteJournals,model=profileModel(),plan=buildSearchPlan(model),collected=[];let scans=0;if(favorites.length){const groups=[];for(let i=0;i<favorites.length;i+=20)groups.push(favorites.slice(i,i+20));for(let g=0;g<groups.length;g++){const ids=groups[g].map(sourceId).join("|"),groupNames=groups[g].map(j=>j.display_name);updateScan({current:groupNames.join(", "),label:"CHECKING JOURNAL",log:"Started search in "+groupNames.join(", ")});$("#feed-status").textContent=`Pass 1/3: testing ${plan.length} query variants in journal group ${g+1}/${groups.length}…`;collected.push(...await fetchTargeted(ids,date,plan,(i,total,item)=>{const p=8+Math.round(42*((g*plan.length+i)/(groups.length*plan.length)));updateScan({progress:p,stage:"Pass 1 of 3 · Query "+i+"/"+total,current:(item?.query||"research concepts")+" · "+groupNames[0],label:"QUERYING",candidates:uniqueWorks(collected).length,queries:g*plan.length+i});$("#feed-status").textContent=`Pass 1/3: query ${i}/${total}, journal group ${g+1}/${groups.length}…`;}));scans+=plan.length;$("#feed-status").textContent=`Pass 2/3: scanning every indexed article in journal group ${g+1}/${groups.length}…`;updateScan({progress:54,title:"Checking every recent article",stage:"Pass 2 of 3 · Complete journal census",current:groupNames.join(", "),label:"CHECKING JOURNAL",candidates:uniqueWorks(collected).length,log:"Targeted queries complete for "+groupNames.join(", ")});collected.push(...await fetchJournalCensus(ids,date,(page,count)=>{updateScan({progress:54+Math.min(32,Math.round(page*4)),stage:"Pass 2 of 3 · Census page "+page,current:groupNames[(page-1)%groupNames.length],label:"CHECKING JOURNAL",candidates:uniqueWorks(collected).length+count});$("#feed-status").textContent=`Pass 2/3: census page ${page}, ${count} records checked…`;}));scans++;}}else{updateScan({current:"All OpenAlex-indexed journals",label:"SEARCH SCOPE",log:"No journal restriction—searching across the full catalog"});collected.push(...await fetchTargeted("",date,plan,(i,total,item)=>{updateScan({progress:8+Math.round(48*i/Math.max(1,total)),stage:"Pass 1 of 3 · Query "+i+"/"+total,current:item?.query||"Research concepts",label:"QUERYING",candidates:uniqueWorks(collected).length,queries:i});$("#feed-status").textContent=`Pass 1/3: query variant ${i}/${total}…`;}));scans+=plan.length;const ranked=uniqueWorks(collected).map(w=>scorePaper(w,model)).filter(Boolean);if(ranked.length<12){const expansions=model.activeFamilies.flatMap(f=>f.terms.slice(3,7).map(query=>({query,stage:"expanded",reason:f.label}))).slice(0,20);updateScan({progress:58,title:"Expanding the search",stage:"Pass 2 of 3 · Scientific synonyms",log:"Initial matches were limited; expanding terminology"});collected.push(...await fetchTargeted("",date,expansions,(i,total,item)=>{updateScan({progress:58+Math.round(27*i/Math.max(1,total)),stage:"Pass 2 of 3 · Synonym "+i+"/"+total,current:item?.query||"Scientific synonym",label:"EXPANDED QUERY",candidates:uniqueWorks(collected).length,queries:plan.length+i});$("#feed-status").textContent=`Pass 2/3: synonym search ${i}/${total}…`;}));scans+=expansions.length;}}const unique=uniqueWorks(collected);updateScan({progress:90,title:"Ranking the strongest matches",stage:"Pass 3 of 3 · Concept-aware relevance",current:"Comparing "+unique.length.toLocaleString()+" unique papers with your profile",label:"FINAL ANALYSIS",candidates:unique.length,log:"Discovery complete at "+unique.length.toLocaleString()+" unique candidate papers"});state.lastSearchReport={queries:plan.length,scans,candidates:unique.length,journals:favorites.length,date};return unique;}
